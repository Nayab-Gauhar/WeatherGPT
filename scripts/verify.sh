...
# --- Tier 2: the language-model path ------------------------------------------
# Reported, not asserted: the Gemini free tier allows 20 requests per model per
# day, so these legitimately fall back to Tier 1 once quota is spent. The point
# is to see WHICH engine served the answer and that the reply is still sensible.
aiask() { # aiask <query> <label> <settle>
  ab fill ".composer__input" "$1" > /dev/null 2>&1
  ab click ".composer__send" > /dev/null 2>&1
  sleep "${3:-26}"
  ab eval "(() => {
    const m=[...document.querySelectorAll('.msg--bot')].pop();
    return JSON.stringify({
      engine: m.querySelector('.msg__engine') ? 'TIER-2 (Gemini)' : 'TIER-1 (fallback)',
      note: m.querySelector('.msg__degraded')?.innerText?.slice(0,70) || null,
      cards: [...m.querySelectorAll('.card')].length,
      answer: (m.querySelector('.msg__text')?.innerText||'').replace(/\n+/g,' ').replace(/Generated by.*/,'').slice(0,280)
    }, null, 1);
  })()" 2>&1 | tail -9
  ab screenshot "$SHOTS/$2.png" 2>&1 | tail -1
}

echo "== 9b. AI: two-city comparison with personal context =="
aiask "Compare Delhi and Bengaluru for a morning run tomorrow, I have mild asthma" "v9b-ai-compare" 28

echo "== 9c. AI: multi-step, past and future in one question =="
aiask "Is this September wetter than normal in Nagpur, and will next week continue the trend?" "v9c-ai-multistep" 28
